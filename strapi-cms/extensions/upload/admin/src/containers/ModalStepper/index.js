...
const ModalStepper = ({
  ...
  const [showModalConfirmButtonLoading, setShowModalConfirmButtonLoading] =
    useState(false);
  ...
  const { currentStep, fileToEdit, filesToDownload, filesToUpload } =
    reducerState.toJS();
  const {
    Component,
    components,
  ...
  const handleSubmitEditExistingFile = async (
    ...
    const requestURL = shouldDuplicateMedia
      ? `/${pluginId}`
      : `/${pluginId}?id=${id}`;
  ...
  const handleUploadFiles = async () => {
    ...
    const requests = filesToUpload.map(
      async ({
        ...
        try {
          // CUSTOM PROGRAMMIER.BAR CODE START ------------
          const isDevelopment = window.location.hostname === 'localhost';
          const isImage = file.type.includes('image');
          const isNotBiggerThan10Mb = file.size < 10485760;

          if (isDevelopment || isImage || isNotBiggerThan10Mb) {
            // CUSTOM PROGRAMMIER.BAR CODE END ------------
            formData.append('files', file);
            formData.append('fileInfo', JSON.stringify(fileInfo));
            ...
            const hash = `${file.name.replace(ext, '')}_${Math.random()
              .toString(36)
              .substr(2, 5)}`;
            const storageBaseUrl = 'https://storage.googleapis.com';
            const storageBucket = 'programmier-bar-strapi';
            const storagePath = `${hash}/${hash}${ext}`;
            const storageRequestUrl = `https://storage.googleapis.com/${storageBucket}/${encodeURIComponent(
              storagePath
            )}`;

            // Request Cloud Storage access token
            const { token } = await request(`/${pluginId}/cloud-storage-token`);

            // Upload file to cloud storage
            await fetch(storageRequestUrl, {
              method: 'PUT',
              headers: {
            ...
            formData.append(
              ...
              JSON.stringify({
                ...
                mime: file.type,
                size: file.size / 1000,
                url: `${storageBaseUrl}/${storageBucket}/${storagePath}`,
              })
            );

            // Create file at strapi
            await request(
              `/${pluginId}/files`,
              {
                method: 'POST',
          ...
          <section>
            ...
            {currentStep === 'edit' && (
              <div style={{ margin: 'auto 0' }}>
                <Button
                  disabled={
                    isFormDisabled || areButtonsDisabledOnEditExistingFile
                  }
                  color="primary"
