...
export default defineHook(({ action }, hookContext) => {
    ...
    logger.info(
        `${HOOK_NAME}: Initialized with platforms: ${[hasBluesky && 'Bluesky', hasMastodon && 'Mastodon'].filter(Boolean).join(', ')}`
    )
    ...
    action(
        'social_media_posts.items.update',
            ...
            for (const postId of keys) {
                try {
                    ...
                    if (result) {
                        ...
                        logger.info(
                            `${HOOK_NAME}: Successfully published post ${postId} to ${post.platform}: ${result.postUrl}`
                        )
                    }
                } catch (err: any) {
