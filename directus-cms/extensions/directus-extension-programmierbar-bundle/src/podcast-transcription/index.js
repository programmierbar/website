...
module.exports = ({ schedule, action }, config) => {
    ...
    action('podcasts.items.create', ({ payload, ...metadata }, context) => trigger({ payload, metadata, context }))
    action('podcasts.items.update', ({ payload, ...metadata }, context) => trigger({ payload, metadata, context }))
}
