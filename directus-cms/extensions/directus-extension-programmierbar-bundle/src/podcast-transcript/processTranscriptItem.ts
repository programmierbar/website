...
function processTranscriptItem(HOOK_NAME: string, { logger, ItemsService, getSchema, env }: Dependencies) {
    return async () => {
        ...
        const existingTranscripts = await transcriptItemsService.readByQuery({
            filter: { raw_response: { _null: true } },
            sort: ['-date_created'],
            limit: 1,
        })
        if (existingTranscripts.length === 0) {
            logger.info(`${HOOK_NAME} hook: Found no waiting transcripts. ` + `Exiting hook early.`)
            return
        }
        ...
        if (!env.DEEPGRAM_API_URL) {
            logger.info(`${HOOK_NAME} hook: DEEPGRAM_API_URL env variable not set. ` + `Exiting hook early.`)
            return
        }
        ...
        try {
            ...
            existingTranscript.supported_features = ['timestamps', 'diarization']
