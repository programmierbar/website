      ...
      await Promise.all(
        items.map(async (item) => {
          ...
          if (!transcription_export_id) {
            const isTranscriptionDone = await transcription
              .isDone(transcription_id, env)
              .catch((e) => {
            ...
            if (!isTranscriptionDone) {
              logger.info(
                `${HOOK_NAME} hook: Transcript for "${item.title}" is not ready yet.`
              );
              return;
            }

            const exportId = await transcription.createExport(
              transcription_id,
              env
            );
            ...
            logger.info(
              `${HOOK_NAME} hook: Transcript Export for "${item.title}" is triggered.`
            );
          } else {
            const transcript = await transcription
              .getExportUrl(transcription_export_id, env)
              .catch((e) => {
                logger.error(`${HOOK_NAME} hook: Error: ${e.message}`);
                return null;
              });
            ...
            if (!transcript) {
              logger.info(
                `${HOOK_NAME} hook: Transcript Export for "${item.title}" is not ready yet.`
              );
              return;
            }
            ...
            logger.info(
              `${HOOK_NAME} hook: Transcript for "${item.title}" is created.`
            );
          }
        })
      );
