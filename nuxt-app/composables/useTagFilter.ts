 ...
 */
export function useTagFilter<Entities extends { tags: Pick<TagItem, 'id' | 'name'>[] }>(
    ...
    const getTags = (prevTags?: Tag[]) => {
        ...
        const tags = input.value?.reduce((tagList, entity) => {
            ...
            entity.tags.forEach((entityTag) => {
                ...
                if (existingTag) {
                    ...
                    tagList.push({
                        ...entityTag,
                        is_active: prevTags?.find(({ id }) => id === entityTag.id)?.is_active || false,
                        occurrence: 1,
                    })
                }
            })
        ...
        return tags?.sort((a, b) => (a.occurrence < b.occurrence ? 1 : -1)).slice(0, 25) || []
    }
    ...
    const output = computed(() => {
        ...
        if (activeTags.length) {
            return input.value!.filter((entity) =>
                activeTags.some((activeTag) => entity.tags.find((tag) => tag.id === activeTag.id))
            )
        }
