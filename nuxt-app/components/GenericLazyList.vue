...
const style = computed(() => {
    const paddingStart = paddingStartList.value.reduce((total, size) => total + size, 0) + 'px'
    const paddingEnd = paddingEndList.value.reduce((total, size) => total + size, 0) + 'px'
 ...
 */
const addElement = (scrollDirection: 'up' | 'down', index: Ref<number>, paddingList: Ref<number[]>) => {
...
const removeElement = (
    ...
    if (itemElement.value) {
        ...
        const { height, width, top, right, bottom, left } = itemElement.value.getBoundingClientRect()
        ...
        if (
            ...
            (scrollDirection === 'down' && (props.direction === 'vertical' ? bottom : right) < 0)
        ) {
            // Destructure computed style object
            const { marginTop, marginRight, marginBottom, marginLeft } = window.getComputedStyle(itemElement.value)
...
const handleScroll = () => {
    ...
    const nextScrollPosition = props.direction === 'vertical' ? scrollTop || scrollY : scrollLeft || scrollX
    const scrollDirection = lastScrollPosition > nextScrollPosition ? 'up' : 'down'
    ...
    if (scrollDirection === 'up' && firstIndex.value > 0 && firstItemElement.value) {
        // Destructure client rect of first item element
        const { bottom, right } = firstItemElement.value.getBoundingClientRect()
        ...
        if (
            ...
            removeElement(scrollDirection, lastIndex, lastItemElement as Ref<HTMLElement>, paddingEndList)
        }
        ...
        if (
            ...
            removeElement(scrollDirection, firstIndex, firstItemElement as Ref<HTMLElement>, paddingStartList)
        }
    }
}
...
watch([() => props.items, lazyListElement, firstIndex, lastIndex], () => {
    nextTick(() => {
        firstItemElement.value = lazyListElement.value?.firstElementChild as HTMLElement
        lastItemElement.value = lazyListElement.value?.lastElementChild as HTMLElement
    })
})
</script>
