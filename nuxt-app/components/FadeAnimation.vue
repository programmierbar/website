<template>
    <div
        ...
        class="transition"
        :class="[
            isVisible || (isFadeIn && fadeIn === 'none') || (!isFadeIn && fadeOut === 'none')
                ? 'opacity-100'
                : 'opacity-0',
            !isVisible &&
                isFadeIn &&
                ((fadeIn === 'from_top' && '-translate-y-16 md:-translate-y-20 lg:-translate-y-28') ||
                    (fadeIn === 'from_right' && 'translate-x-16 md:translate-x-20 lg:translate-x-28') ||
                    (fadeIn === 'from_bottom' && 'translate-y-16 md:translate-y-20 lg:translate-y-28') ||
                    (fadeIn === 'from_left' && '-translate-x-16 md:-translate-x-20 lg:-translate-x-28')),
            !isVisible &&
                !isFadeIn &&
                ((fadeOut === 'to_top' && '-translate-y-16 md:-translate-y-20 lg:-translate-y-28') ||
                    (fadeOut === 'to_right' && 'translate-x-16 md:translate-x-20 lg:translate-x-28') ||
                    (fadeOut === 'to_bottom' && 'translate-y-16 md:translate-y-20 lg:translate-y-28') ||
                    (fadeOut === 'to_left' && '-translate-x-16 md:-translate-x-20 lg:-translate-x-28')),
        ]"
        :style="{
...
export default defineComponent({
    props: {
        fadeIn: {
            type: String as PropType<'normal' | 'from_top' | 'from_right' | 'from_bottom' | 'from_left' | 'none'>,
            default: 'none',
        },
        fadeOut: {
            type: String as PropType<'to_top' | 'to_right' | 'to_bottom' | 'to_left' | 'none'>,
            default: 'none',
        },
        duration: {
            type: Number,
    ...
    setup(props) {
        ...
        const observationTarget = computed(() =>
            props.fadeIn !== 'none' || props.fadeOut !== 'none' ? fadeElement.value : null
        )
